#!/bin/bash
# Owner-authorized SAFE-01 run only; preserve the one-launch receipt and stop.
set -euo pipefail
factory_dir=$(cd -- "$(dirname -- "$0")" && pwd)
python3 "$factory_dir/install.py" --safe01
finish() {
...
printf '%s\n' 'SAFE-01 exceeded ten-minute observation limit; stopping without retry.' >&2
exit 1
